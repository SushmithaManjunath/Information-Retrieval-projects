...
for f,x in enumerate(data1):
    ...
    data2.append(zz)
    all_token.extend(zz)
    with open(os.getcwd()+'/Outputs/'+str(f)+".data","w") as f :
        f.writelines(zz)  
...
with open(os.getcwd()+'/Outputs/'+'/sorted_by_token.data',"w") as f :
        f.writelines(sorted_tokens) 
with open(os.getcwd()+'/Outputs'+'/sorted_by_value.data',"w") as f :
...
plt.savefig(os.getcwd()+'/Outputs/'+'graph_time.png')
plt.show()